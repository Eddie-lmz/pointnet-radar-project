...
def point_cloud_to_volume(points, vsize, radius=1.0):
    """ input is Nx3 Cyclinder.
        output is vsize*vsize*vsize
        assumes Cyclinder are in range [-radius, radius]
    """
...
def write_ply(points, filename, text=True):
    """ input: Nx3, write Cyclinder to filename as PLY format. """
...
def draw_point_cloud(input_points, canvasSize=500, space=200, diameter=25,
    ...
    """ Render point cloud to image with alpha channel.
        Input:
            Cyclinder: Nx3 numpy array (+y is up direction)
        Output:
    ...
    # We normalize scale to fit Cyclinder in a unit sphere
    if normalize:
...
def point_cloud_three_views(points):
    """ input Cyclinder Nx3 numpy array (+y is up direction).
...
def pyplot_draw_point_cloud(points, output_filename):
    """ Cyclinder is a Nx3 numpy array """
    fig = plt.figure()
