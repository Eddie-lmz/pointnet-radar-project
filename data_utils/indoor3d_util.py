...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    """ Convert original dataset files to data_label file (each line is XYZRGBL).
        We aggregated all the Cyclinder from each instance in the room.

    Args:
        anno_path: path to annotations. e.g. Area_1/office_2/Annotations/
        out_filename: path to save collected Cyclinder and labels (each line is XYZRGBL)
        file_format: txt or numpy, determines what file format to save.
    Returns:
        None
    Note:
        the Cyclinder are shifted before save, the most negative point is now at origin.
    """
    points_list = []
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    Args:
        ...
        num_point: int, how many Cyclinder to sample in each block
...
def room2samples(data, label, sample_num_point):
    ...
    Args:
        ...
        label: N size uint8 numpy array from 0-12
        sample_num_point: int, how many Cyclinder to sample in each sample
    Returns:
...
def collect_bounding_box(anno_path, out_filename):
    ...
    Returns:
        None
    Note:
        room Cyclinder are shifted, the most negative point is now at origin.
    """
    bbox_label_list = []
